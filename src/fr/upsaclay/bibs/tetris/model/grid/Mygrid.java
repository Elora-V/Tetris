...
public class Mygrid implements TetrisGrid,TetrisGridView {
    ...
    // Cette classe représente la grille de jeu Tetris.

    //////////////////// Elements de classe ///////////////////////////
    TetrisCell[][] Mygrid_case; // grille composé de cellules de tetromino
    int numligne;  // nombre de lignes de la grille
    int numcolonne;  // nombre de colonne de la grille

    Tetromino tet;  // un tetromino qui vient se positionné par dessus la grille, celui-ci peut bouger

    TetrisCoordinates tcoord; // coordonnées du tetromino qui peut bouger
    // on associe le coin supérieur gauche du tetromino à une case dans la grille
    ...
    public Mygrid(int numligne, int numcolonne) {
        ...
        // on remplit la grille par des cases vides :
    ...
    public boolean hasConflicts () {
        ...
        // on regarde si il y a une cellule non vide sous celle non vide du tetromino :
        for (int i = itetromino; i <= itetromino - 1 + tet.getBoxSize(); i++) {
            for (int j = jtetromino; j <= jtetromino - 1 + tet.getBoxSize(); j++) {

                // si la case du tetromino est non vide :
                if (tet.cell(i - itetromino, j - jtetromino) != TetrisCell.EMPTY) {
                    if (i >= 0 && i < numligne && j >= 0 && j < numcolonne) { // (on verifie qu'on sort pas de la grille)
                        // si la case de la grille est non vide :
                        if (gridCell(i, j) != TetrisCell.EMPTY) {
                            // alors il y a conflict
                            return true;
                        }
                    } else {
                        // si on sort de la grille : il y a conflict
                        return true;
                    }
                }
            }
    ...
    // La méthode suivante est utilisé pour voir la projection des tetromino dans le bas de la grille.
    public TetrisCell projectionVisibleCell(int i, int j){
        // on calcul les coordonées du tetromino dans le cas où on ferait un hardDrop
    ...
    public boolean tryMove (TetrisCoordinates dir){
        ...
        // on conserve l'ancienne coordonnée du tetromino
        int Old_line = tcoord.getLine();
        int Old_col = tcoord.getCol();

        // on bouge le tetromino
        ...
        // si cela entraine un conflict on remet l'ancienne position : on ne peut pas bougers
        if (hasConflicts()) {
